...
DebugInfoFrame::DebugInfoFrame() {
	...
	m_layout.SetDirection(gui::LinearLayout::eDirection::VERTICAL);
	m_layout.SetInverted(true);
	ShowBackground(false);

	gui::ControlStyle textStyle = GetStyle();
...
void DebugInfoFrame::Update(float elapsed) {
	...
	m_frameBufferLabel.SetText(EncodeString<char32_t>(ss.str()));
}
...
void DebugInfoFrame::SetAdapterInfo(const gxapi::AdapterInfo& info) {
	m_adatperInfo = info;

	std::stringstream ss;
	ss << info.name << " -- "
	   << (info.dedicatedVideoMemory / 1048576) << " MiB VRAM";
	m_videoCardLabel.SetText(EncodeString<char32_t>(ss.str()));
}


void DebugInfoFrame::SetResolutionInfo(Vec2u resolution) {
