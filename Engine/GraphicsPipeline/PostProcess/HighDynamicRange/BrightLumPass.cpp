...
void BrightLumPass::Setup(SetupContext& context) {
	...
	if (!m_PSO) {
		...
		psoDesc.renderTargetFormats[0] = m_brightPassRtv.GetResource().GetFormat();
		psoDesc.renderTargetFormats[1] = m_luminanceRtv.GetResource().GetFormat();

		m_PSO.reset(context.CreatePSO(psoDesc));
	}

	this->GetOutput<0>().Set(m_brightPassRtv.GetResource());
	this->GetOutput<1>().Set(m_luminanceRtv.GetResource());
}
...
void BrightLumPass::Execute(RenderContext& context) {
	...
	uniformsCBData.brightPassThreshold = 0.8f;
	...
	*/
	...
	commandList.SetResourceState(m_brightPassRtv.GetResource(), gxapi::eResourceState::RENDER_TARGET);
	commandList.SetResourceState(m_luminanceRtv.GetResource(), gxapi::eResourceState::RENDER_TARGET);
	...
	RenderTargetView2D* pRTV[] = { &m_brightPassRtv, &m_luminanceRtv };
	commandList.SetRenderTargets(2, pRTV, 0);

	gxapi::Rectangle rect{ 0, (int)m_brightPassRtv.GetResource().GetHeight(), 0, (int)m_brightPassRtv.GetResource().GetWidth() };
