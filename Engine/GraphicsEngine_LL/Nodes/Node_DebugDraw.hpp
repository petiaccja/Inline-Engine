	...
	public:
		...
		void AddSphere(mathfu::Vector3f pos, float radius, int life)
		{
			for (int c = 0; c < objects.size(); ++c)
			{
				...
				{
					objects[c] = DebugSphere(pos, radius, life);
					return;
				}
			}

			objects.push_back(DebugSphere(pos, radius, life));
		}
		...
		void AddCross(mathfu::Vector3f pos, float size, int life)
		{
			for (int c = 0; c < objects.size(); ++c)
			{
				if (!IsAlive(objects[c].GetLife()))
				{
					objects[c] = DebugCross(pos, size, life);
					return;
				}
			}

			objects.push_back(DebugCross(pos, size, life));
		}
		...
		void AddLine(mathfu::Vector3f start, mathfu::Vector3f end, int life)
		{
			for (int c = 0; c < objects.size(); ++c)
			{
				if (!IsAlive(objects[c].GetLife()))
				{
					objects[c] = DebugLine(start, end, life);
					return;
				}
			}

			objects.push_back(DebugLine(start, end, life));
		}
		...
		void AddBox(mathfu::Vector3f min, mathfu::Vector3f max, int life)
		{
			for (int c = 0; c < objects.size(); ++c)
			{
				if (!IsAlive(objects[c].GetLife()))
				{
					objects[c] = DebugBox(min, max, life);
					return;
				}
			}

			objects.push_back(DebugBox(min, max, life));
		}
						...
						int life)
		{
			...
			objects.push_back(DebugFrustum(
				newNearLowerLeft,
				newNearUpperLeft,
				newNearLowerRight,
				newFarLowerLeft,
		...
		const std::vector<DebugObject>& GetObjects()
		{
			return objects;
		}
	};
