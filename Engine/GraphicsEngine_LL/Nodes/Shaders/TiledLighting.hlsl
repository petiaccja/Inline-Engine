						...
						)
{
	...
	uint2 group_size = uint2(global_size) / local_size;
	float2 texel = global_id / global_size;

	uint local_num_of_lights = lightCullData.Load(int3(group_id.x * group_size.y + group_id.y, 0, 0));

	float3 color = float3(0, 0, 0);
	for (uint c = 0; c < local_num_of_lights; ++c)
	{
		uint index = lightCullData.Load(int3(group_id.x * group_size.y + group_id.y, c + 1, 0));
		...
		if (attenuation > 0.0)
		{
			...
			color += (n_dot_l * attenuation) * (diffuse_color.xyz * albedo.xyz); //TODO: shadow
		}
	}
