...
void NeighborMax::Setup(SetupContext& context) {
	...
	m_inputTexSrv.GetResource().SetName("Motion blur neighbormax input tex SRV");
	...
	if (!m_fsq.HasObject()) {
		...
		m_fsq.SetName("Motion blur neighbormax full screen quad vertex buffer");
		m_fsqIndices = context.CreateIndexBuffer(indices.data(), sizeof(uint16_t)*indices.size(), indices.size());
		m_fsqIndices.SetName("Motion blur neighbormax full screen quad index buffer");
	}
...
void NeighborMax::InitRenderTarget(SetupContext& context) {
	if (!m_outputTexturesInited) {
		...
		m_neighbormax_rtv.GetResource().SetName("motion blur neighbormax RTV");
	}
}
