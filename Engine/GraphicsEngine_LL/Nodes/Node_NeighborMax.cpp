...
void NeighborMax::Setup(SetupContext& context) {
	...
	m_inputTexSrv.GetResource()._GetResourcePtr()->SetName("Motion blur neighbormax input tex SRV");
	...
	if (!m_fsq.HasObject()) {
		...
		m_fsq._GetResourcePtr()->SetName("Motion blur neighbormax full screen quad vertex buffer");
		m_fsqIndices = context.CreateIndexBuffer(indices.data(), sizeof(uint16_t)*indices.size(), indices.size());
		m_fsqIndices._GetResourcePtr()->SetName("Motion blur neighbormax full screen quad index buffer");
	}
...
void NeighborMax::InitRenderTarget(SetupContext& context) {
	if (!m_outputTexturesInited) {
		...
		neighbormax_tex._GetResourcePtr()->SetName("Motion blur neighbormax tex");
		m_neighbormax_rtv = context.CreateRtv(neighbormax_tex, formatNeighborMax, rtvDesc);
		m_neighbormax_rtv.GetResource()._GetResourcePtr()->SetName("motion blur neighbormax RTV");
	}
}
