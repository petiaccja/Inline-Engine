...
class ResourceResidencyQueue {
	struct Task {
		Task() = default;
		Task(std::vector<GenericResource*> resources, SyncPoint syncPoint)
...
public:
	...
	SyncPoint EnqueueInit(std::vector<GenericResource*> resources);
	...
	void EnqueueClean(SyncPoint waitFor, std::vector<GenericResource*> resources, CleanObjectT&&... cleanObjects);
...
template<class... CleanObjectT>
inline void ResourceResidencyQueue::EnqueueClean(SyncPoint waitFor, std::vector<GenericResource*> resources, CleanObjectT&&... cleanObjects) {
	struct SpecialTask : Task {
		SpecialTask() = default;
		SpecialTask(std::vector<GenericResource*> resources, SyncPoint syncPoint, CleanObjectT&&... cleanObjects)
