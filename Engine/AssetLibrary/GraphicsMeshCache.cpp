...
void GraphicsMeshCache::Reload(gxeng::IMesh& asset, const std::filesystem::path& path) {
	Model model{ path };
	...
	asset.Set(vertices.data(), &vertices[0].GetReader(), vertices.size(), indices.data(), indices.size());
}
