...
float4 PSMain(PS_Input input) : SV_TARGET {
	...
	for (int i = 0; i < int(uniforms.reconstructionFilterTaps); ++i) {
		...
		float alphaY = SoftDepthCompare(currDepth, depthSampleY) * Cone(distCurrSample, templVelSampleY) + SoftDepthCompare(depthSampleY, currDepth) * Cone(distCurrSample, tempVel) + Cylinder(distCurrSample, templVelSampleY) * Cylinder(distCurrSample, tempVel) * 2.0;
