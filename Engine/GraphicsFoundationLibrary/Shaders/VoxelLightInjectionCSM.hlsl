...
float3 getCSMTexelWSPos(float depth, float3 camPos, float3 camViewDir, float3 camUpDir, float3 extents, float2 texCoord) {
	...
	float3 wsPos = camPos + depth * extents.z * camViewDir + (texCoord.x - 0.5) * extents.x * camRightDir + (1 - texCoord.y - 0.5) * extents.y * camUpDir;
...
void PSMain(PS_Input input) {
	...
	for (int c = 0; c < 4; ++c) { //for each cascade
